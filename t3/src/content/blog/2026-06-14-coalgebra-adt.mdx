...
I'm using algebraic data types (ADTs) on this blog extensively, and while the term is floating around in programming circles quite a lot, I feel like nobody really understand what it means, so let me clear up the confusion for you.

ADTs are like an abstract way to describe complex types, without us needing to be concerned about the specifics of any specific programming language. Say we are in typescript with a set of primitive types like `string`, `number`, `boolean`, and we want to make more complex types out of them, then what are we usually doing?
...
This, along with a type for specifying functions, is all you need to model pretty much anything in any programming language. As a result, most programming languages feature ADTs in some form or another, though some languages are more direct about it than others.

Without further ado lets keep this one short and terse and hop right into it.

## Product Types

Say you have an integer and a string and you would like two store both in a single variable. How do you do it?
...
We say that all of those representations are **the product** of an integer and a string, and we even write this type as `number * string`, with the multiplication symbol.
...
Discriminated unions are common in API design, but it might be foreign to some. So to clarify, a discriminated union is a regular union, given you can tell the "branches" apart. Take for example the type `T + 1`, which in non-nerd speak means `Nullable<T>`:
...
Given any value of our type `T + 1`, you can immediately tell if it's a `T` or a `null` by doing a null check:
...
Again, what the extra prop's value is called or what it's value is, is irrelevant -- it's just a mechanical crutch for translating the maths description of `string + string` into a typescript type.

There is by the way another common implementation of sum types in a lot of programming languages: subclasses!
...
The discriminator in this case can be checked via `typeof` or checking the `.constructor.name` property.
...
Without going into too much detail, we can say that when a type definition contains a number like that, we mean it to be the "enum that contains N values".

For example, when we write the type `string * 2`, then the `2` refers to an enum that contains 2 values. Again, the actual names in the enum don't matter to us, they are all equivalent for our purposes. That being said, a popular choice for names are `true` and `false`, which is why we often call `2` the type of "booleans".
...
```typescript
type TwoTimesString = [boolean, string];
type StringPlusString = 
 | { t: "left", value: string }
