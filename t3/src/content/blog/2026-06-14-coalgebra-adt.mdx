...
I feel like the term "algebraic data type" is floating around programming circles quite a lot, but nobody really understand what it means, so let me clear up the confusion for you.

In a nutshell, ADTs give us a way to specify types, without us having to dig into the specifics of any particular programming language. ADTs use the `+` and `*` symbols to combine types together, as opposed to whatever niche syntax is available to us in programming like java's `abstract class {}` or ruby's `<< self` or swift's `extension Int {}`. Now, it may seem like a statement like `number * string * boolean` is maths and maths is hard, but let me assure you it's not rocket science.
