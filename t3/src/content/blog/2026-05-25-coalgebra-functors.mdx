...
**Composition Law**: `list.map(x => f(g(x))) === list.map(g).map(f)`, or, if you have two functions and you wish to apply both, you can either do two passes over the list, or one, and it should yield the same result. This forces `map` to be "honest" -- it can only apply the function once to each element, and not inspect the function to do sneaky manipulations. 
