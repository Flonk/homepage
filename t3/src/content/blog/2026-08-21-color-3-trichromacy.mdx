...
<DeckCard vizProps={{ phase: "cones", vision: "normal" }}>
  ...
  And even in normal vision, we have a bit of a color blindness situation on the red end of the spectrum, where the reds all look very similar across a 100 or so nanometers, because the M and L signals are so similar.
...
<DeckCard vizProps={{ phase: "cones", vision: "normal" }}>
  ...
  This works by stimulating each of the three cones in a way that is indistinguishable from the way they would be stimulated by a real photon of that color. This is called *metamerism*, and it is the reason why we can see all the colors on our screens, even though they are not real.
