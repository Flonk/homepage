...
AbstractDtoFactory? What is it's essence?", and then we spend hours on delineating
what does and what doesn't constitute the essence of an AbstractDtoFactory, what
to put int that class and what to put somewhere else. It really makes me feel like a
...
What would the type of a fold be though? It takes
an initial value, and a function that merges an element of the list and the
accumulator, and then we return the next accumulator. If the list contains
`Int`s, and the accumulator is of any type `R`, then the type of `fold` would be:
...
it takes a while to build intuition.
...
Feel free to implement this Church encoding of lists in typescript
to get a better feel, it really is a fun exercise. You'll see how the data propagates
through your program and you can slowly trace which funtions call which. And you'll get to
really learn functional programming concepts along the way!
