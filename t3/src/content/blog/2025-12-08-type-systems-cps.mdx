...
In javascript we were calling continuations "callbacks", but it's the same idea:
...
If we were to write a version of `readFile` that instead of returning `Void`
is generic in it's return type, like `∀R. FilePath -> Encoding -> (Error -> String -> R) -> R`, we do start to see similarities to `apply42`'s type, however.
...
I don't really want to make you read scheme code, so let's assume we have a 
...
things with it. Do not worry if this is too complicated, you won't need this
later -- I just want to show you cool stuff.
...
nto `k` -- that is to say, we split our program into two parts: the part
...
We log `result` to the console, which prints `0`, and then we call `saved(result + 1)`, which **jumps back in time** to our `$$$` marker, but instead of rerunning the `callCC` expression, that line becomes `let result = result + 1` and from then on 
...
if statements, loops, you name it -- they can all be implemented using continuations.
