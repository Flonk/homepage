...
could be `Horse`. But there definitely _is_ some type `A` such that `getAny` has it.
Pinky promise.
...
is a list of _something_ -- "there exists a type", but I don't know which one.
...
_also_ turn out to be two sides of the same coin (via the Curry-Howard isomorphism),
so types can also be skolemized.
...
chosen by the code that created the `Showable`, which might have happened much earlier.
...
Refer also to **opaque types** or **opaque pointers** which are sometimes talked about
in API design. It's a similar idea!
