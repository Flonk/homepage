...
{/* The card is about the range, so the wave runs the range: infrared up to
    the near end of X-rays and back. The bottom is past where the drawing is
    comfortable — a wave down there is wider than a laptop pane — which is
    fair enough on a card about how much spectrum there is. */}
<DeckCard vizProps={{ sweep: [80, 6000] }}>
