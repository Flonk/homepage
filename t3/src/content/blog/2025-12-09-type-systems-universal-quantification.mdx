...
But there is one thing all the generic function we've looked at so far have in
...
is the only one we deal with in day-to-day programming. But System F is actually 
a lot more powerful than that.

Since this is where we leave the comfort zone of "working man's knowledge", we 
will slow the pace down a lot to explore this new concept.

From now on I will also drop explicit type annotations in function definitions,
if they can be inferred from context, to reduce visual noise. (This is problematic
in System F but we'll talk about that later.)
...
Bonus question: `apply10` isn't a particularly useful function. Why?
...
Let me show you an actually useful example of a rank-2 type aswell, assuming
...
This function takes a path to a file, aswell a continuation (which we've
discussed in post #9). So `withFile` will do all the file opening logic and then
hand off the opened file handle to it's continuation.
...
not blow up your program, as long as you don't try to access the result. That is to say, the 
...
Anyway -- why is genericising the return type like that useful? 

Well -- since your continuation must return an error, it can in particular 
**not return the file handle, or leak it outside of itself in sneaky ways**.
...
global state so you can use it later, then Haskell would force you to return a value
...
file, and `withFile` can close it afterwards. In so many other
