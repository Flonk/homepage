...
so let me throw you a pop quiz real quick. One word about notation: 
...
>
  True! In typescript, every type except `null` and `undefined` is a subtype of `object`.
</Question>
...
a subtype of `Animal` -- we must explicitly declare the relationship with `extends`.
...
to the syntactic rules for subtyping we discussed above, like covariance in return types
and contravariance in argument types, there is also a semantic aspect to LSP.

LSP says that if `S <: T`, then any property that holds for values of type `T`
must also hold for values of type `S`, where "any property" is very broad,
and not really enforcible by a compiler. If you have for example a File class with a
`close()` method, and you create a "subtype" that also has a `close()` method with the
same signature but doesn't actually close the file, then you have violated LSP, even though
your type checks just fine.
...
least can manually decide whether LSP holds or not. All in all I find that argument to not
...
the same use cases, though the mechanics are very different. Haskell is a really interesting
language in general, everyone should try it for a few weeks! Rust, being very FP inspired,
also only does very limited subtyping, for lifetimes.
