...
much like Schrödinger's cat, it's often misunderstood. So, just to be annoying,
...
<Question n={1} question={`number <: string`}>
  Obviously not. Numbers are not strings.
</Question>

<Question n={2} question={`{ foo: number, bar: string } <: { foo: number }`}>
...
<Question n={4} question={`given A <: B: { foo: A } <: { foo: B }`}>
  True! This is called **covariance**.
</Question>

<Question n={5} question={`given A <: B: ((a: A) => void) <: ((b: B) => void)`}>
...
<Question n={6} question={`number <: object`}>
...
In my opinion this is a direct result of over-OOPing, and typescript does it much better, I'm sorry.
...
A method of `BankAccount` can obviously access its own private `balance` field, but
...
least manually decide whether LSP holds or not. All in all I find this argument to not
be very strong, as in practice, structural subtyping is just so much more convenient.
