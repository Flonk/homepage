...
Given an average programming language, we can deduce that a top type must demand
*nothing* of its values, because as soon as you require a single property `foo`
to `Top`, any `foo`less type would no longer be a subtype of that enriched `Top`.

And precisely because `Top` imposes no requirements, every value passes the bar --
`42`, `"foo"`, `{ a: 1 }`, `[]`, you name it.
