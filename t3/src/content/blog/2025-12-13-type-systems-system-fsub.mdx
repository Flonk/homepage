...
Turns out we just need to two more ingredients: **bounded quantification** and
a **top type**.
...
to it, any `foo`less type would no longer be a subtype of it.
...
degenerate cases too: For example, the function `absurd :: Bottom → String`
...
because it doesnt have intersection and union types built in like typescript
does, but as always, people have defined extensions to
System F\<: that add these features; refer to for example "Programming with
