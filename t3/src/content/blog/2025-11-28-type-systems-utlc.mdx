---
title: "A Hierarchy of Type Systems #1: Untyped Lambda Calculus"
description: ""
...
- **Variables**: Im a fan of `x` and `y` myself, but you can use any names or symbols you like.
- **Functions**: `λx. y` is a function that takes an argument `x` and returns `y`. In javascrpt, this would be written as `(x) => y`.
...
We call the labmda, which means we substitute `5` for `x` in the body of
...
We pulled the numbers above, aswell as the `add` function, out of thin air.
But even without "special" symbols and function, we can define integers using
...
"branching", or "two-valued-ness", in some abstract sense.
...
And if you have never seen church encodings before, then this lambda term
...
that acts as a basis for all the others. It's so minimalistic that you can't remove
...
We can fix that by introducing types! And we will do so in the next post,
where we will look at the simply typed lambda calculus.
