...
It feels like we have been here before, all the way back in post #4 when we wanted to
an `apply` function that was generic in it's argument. We're running into the exact
same problem, just at the type level.

I think we're finally ready to move to the next level of abstraction: **Type Operators**!
It only took us 12 posts.
...
three list types we defined above and make the `Int`, `Boolean`, and `String`
parts into a variable.

And usually we do that via functions, right? So can't we just write, like...
...
call, but where we use angle brackets instead of parentheses.
