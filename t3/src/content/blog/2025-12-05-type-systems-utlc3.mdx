...
structure -- `map` is actually a function transformer! <sup>[1]</sup>
...
<sub>
**[1]** Of course there are infinitely many functions of type `(A -> B) ->
(A[] -> B[])`. But it turns out that any function of this type can be decomposed
into `map` and some "post-processing" function. Any `f' :: (A -> B) -> (A[] -> B[])`
...
for some function `h :: B[] -> B[]`. This makes the `map` function a sort of
"universal" function transformer for lists.
