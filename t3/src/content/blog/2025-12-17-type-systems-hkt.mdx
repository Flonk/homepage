...
we let's not get into that at this point. A thing to note however is that the function
type that `Natural F G` represents is *generic* in `A` -- it works for any type `A`, so
we can establish a theorems-for-free kind of argument here: The function can't inspect the
type `A` after all, so it can't really do anything outrageous to it and the transformation
