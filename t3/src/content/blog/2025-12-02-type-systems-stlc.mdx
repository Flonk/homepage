...
lambda calculus. The **simply typed lambda calculus** gives the most basic type
...
judgements" aswell: For example, we define `true` and `false` to be `Boolean`;
...
The solution is to **treat `if` as a primitive** aswell, it's a god-given
