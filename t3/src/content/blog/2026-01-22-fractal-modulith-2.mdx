...
sub-sub-app will likely need access to core services like databases and logging for example, so you just pass those down
...
The fractal modulith does this auomatically. Say you are three applications deep in `ReturnsApplication` and suddenly
realize you need to call a method in `SortingService` in the `WarehouseApplication`. To do this the "wrong" way you would
need to call `app.parent.parent.parent.Warehouse.services.Sorting.sortPackages()`,
which is so horrible to look at that no developer would ever do this, and no code reviewer would ever approve it.

The right (and natural) thing to do is to move the `SortingService` up one layer into the `FulfillmentCenterApplication`,
and then pass it down though all the `Returns` applications, so the service that needs it can write `app.services.Sorting.sortPackages()`.
We're naturally incentivising developers to keep the tree structure intact, which is a sign of good architecture.
