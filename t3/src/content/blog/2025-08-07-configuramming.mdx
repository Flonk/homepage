...
It's the age old adage. People write custom languages, so we can
configure their tools in a declarative fashion. Eventually their
tools get popular and suddenly "declarative" config doesn't cut
it anymore, which is when the languages will be retrofitted with
