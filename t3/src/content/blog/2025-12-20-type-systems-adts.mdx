...
Last post we have defined numbers, now let's define some operations.
...
gets a unique label. `string & number` is a non-sensical type
...
makes sure we can tell the variants apart.
...
an isomoprhism, given these types!
...
Algebraic Data Types (ADTs), as they are called, were actually my
first foray into type theory. There is something deeply satisfying
about solving an equation like `2 * string = string + string`, and
then seeing which types that maps to, in say Java.

It really feels like you're refactoring half of your codebase in
in your head in half a second, just by doing basic maths. Like,
`2 * 3 = 6` is such a short statement but would
